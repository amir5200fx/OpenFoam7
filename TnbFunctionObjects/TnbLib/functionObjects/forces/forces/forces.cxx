...
tnbLib::wordList tnbLib::functionObjects::forces::createFileNames
(
...
void tnbLib::functionObjects::forces::writeFileHeader(const label i)
{
	switch (fileID(i))
	{
...
void tnbLib::functionObjects::forces::initialise()
{
	if (initialised_)
	{
		return;
...
tnbLib::functionObjects::forces::devRhoReff() const
{
...
tnbLib::tmp<tnbLib::volScalarField> tnbLib::functionObjects::forces::mu() const
{
...
tnbLib::tmp<tnbLib::volScalarField> tnbLib::functionObjects::forces::rho() const
{
	if (rhoName_ == "rhoInf")
	{
...
tnbLib::scalar tnbLib::functionObjects::forces::rho(const volScalarField& p) const
{
...
void tnbLib::functionObjects::forces::applyBins
(
...
void tnbLib::functionObjects::forces::writeForces()
{
...
void tnbLib::functionObjects::forces::writeBins()
{
	if (nBin_ == 1)
	{
		return;
...
tnbLib::functionObjects::forces::forces
(
	const word& name,
	const Time& runTime,
	const dictionary& dict
...
bool tnbLib::functionObjects::forces::read(const dictionary& dict)
{
	...
	if (porosity_)
	{
		TnbLog << "    Including porosity effects" << endl;
	}
	else
	{
		TnbLog << "    Not including porosity effects" << endl;
	}
...
void tnbLib::functionObjects::forces::calcForcesMoment()
{
	initialise();
...
tnbLib::vector tnbLib::functionObjects::forces::forceEff() const
{
	return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


tnbLib::vector tnbLib::functionObjects::forces::momentEff() const
{
	return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool tnbLib::functionObjects::forces::execute()
{
	return true;
}


bool tnbLib::functionObjects::forces::write()
{
	calcForcesMoment();
