...
tnbLib::pointIndexHit tnbLib::surfaceFeatures::edgeNearest
(
	const point& start,
	const point& end,
	const point& sample
...
tnbLib::List<tnbLib::surfaceFeatures::edgeStatus> tnbLib::surfaceFeatures::toStatus()
const
{
...
void tnbLib::surfaceFeatures::setFromStatus
(
...
)
{
	...
	const scalar minCos = tnbLib::cos(degToRad(180.0 - includedAngle));
...
void tnbLib::surfaceFeatures::calcFeatPoints
(
	const List<edgeStatus>& edgeStat,
	const scalar minCos
)
...
void tnbLib::surfaceFeatures::classifyFeatureAngles
(
...
tnbLib::label tnbLib::surfaceFeatures::nextFeatEdge
(
	const List<edgeStatus>& edgeStat,
...
tnbLib::surfaceFeatures::labelScalar tnbLib::surfaceFeatures::walkSegment
(
	const bool mark,
	const List<edgeStatus>& edgeStat,
...
tnbLib::surfaceFeatures::surfaceFeatures(const triSurface& surf)
	:
	surf_(surf),
	featurePoints_(0),
	featureEdges_(0),
...
tnbLib::surfaceFeatures::surfaceFeatures
(
...
tnbLib::surfaceFeatures::surfaceFeatures(const surfaceFeatures& sf)
	:
	surf_(sf.surface()),
...
tnbLib::labelList tnbLib::surfaceFeatures::selectFeatureEdges
(
...
void tnbLib::surfaceFeatures::findFeatures
(
	const scalar includedAngle,
	const bool geometricTestOnly
)
{
	scalar minCos = tnbLib::cos(degToRad(180.0 - includedAngle));
...
tnbLib::labelList tnbLib::surfaceFeatures::trimFeatures
(
	const scalar minLen,
	const label minElems,
	const scalar includedAngle
...
void tnbLib::surfaceFeatures::writeDict(Ostream& writeFile) const
{
...
void tnbLib::surfaceFeatures::write(const fileName& fName) const
{
	OFstream str(fName);
...
void tnbLib::surfaceFeatures::writeObj(const fileName& prefix) const
{
...
tnbLib::Map<tnbLib::label> tnbLib::surfaceFeatures::nearestSamples
(
...
tnbLib::Map<tnbLib::pointIndexHit> tnbLib::surfaceFeatures::nearestEdges
(
	const labelList& selectedEdges,
...
void tnbLib::surfaceFeatures::nearestSurfEdge
(
	const labelList& selectedEdges,
	const pointField& samples,
...
void tnbLib::surfaceFeatures::nearestFeatEdge
(
	const edgeList& edges,
	const pointField& points,
	scalar searchSpanSqr,   // Search span
...
void tnbLib::surfaceFeatures::operator=(const surfaceFeatures& rhs)
{
...
tnbLib::surfaceFeatures::edgeStatus tnbLib::checkNonManifoldEdge
(
	const triSurface& surf,
	const scalar tol,
	const scalar includedAngle,
...
)
{
	...
	DynamicList<tnbLib::vector> normals(2);
	DynamicList<labelList> bins(2);

	forAll(eFaces, eFacei)
	{
		const tnbLib::vector& n = surf.faceNormals()[eFaces[eFacei]];
	...
	else
	{
		...
		if (includedAngle >= 0)
		{
			...
			forAll(eFaces, i)
			{
				const tnbLib::vector& ni = surf.faceNormals()[eFaces[i]];
				for (label j = i + 1; j < eFaces.size(); j++)
				{
					const tnbLib::vector& nj = surf.faceNormals()[eFaces[j]];
					if (mag(ni & nj) < minCos)
					{
...
void tnbLib::selectManifoldEdges
(
	const triSurface& surf,
	const scalar tol,
	const scalar includedAngle,
