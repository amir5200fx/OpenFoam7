...
tnbLib::scalar tnbLib::pointMVCWeight::tol(small);


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void tnbLib::pointMVCWeight::calcWeights
(
...
) const
{
	...
	forAll(f, j)
	{
		...
		weights[pid] =
			1.0
			/ dist[pid]
			* (tnbLib::tan(theta[jMin1] / 2.0) + tnbLib::tan(theta[j] / 2.0));
...
) const
{
	...
	const tnbLib::cell& cFaces = mesh.cells()[cellIndex_];
	...
	forAll(cFaces, iter)
	{
		...
		forAll(f, j)
		{
			...
			scalar angle = 2.0*tnbLib::asin(l / 2.0);
		...
		forAll(f, j)
		{
			...
			sum +=
				1.0
				/ tnbLib::tan(theta[j])
				* (tnbLib::tan(alpha[j] / 2.0) + tnbLib::tan(alpha[jMin1] / 2.0));
		}
...
tnbLib::pointMVCWeight::pointMVCWeight
(
	const polyMesh& mesh,
	const vector& position,
