...
#include <meshRefinement.hxx>  // added by amir

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

// Calculate inverse sum of edge weights (currently always 1.0)
void tnbLib::snappyLayerDriver::sumWeights
(
...
void tnbLib::snappyLayerDriver::smoothField
(
...
) const
{
	...
	for (label iter = 0; iter < nSmoothDisp; iter++)
	{
		...
		if ((iter % 10) == 0)
		{
			...
			(
				/*meshMover.mesh(),*/  // Edited by amir
				isMasterPoint,
				/*meshPoints,*/  // Edited by amir
				mag(field - average)()
			);
...
void tnbLib::snappyLayerDriver::smoothPatchNormals
(
	const motionSmoother& meshMover,
	const PackedBoolList& isMasterPoint,
	const PackedBoolList& isMasterEdge,
...
void tnbLib::snappyLayerDriver::smoothNormals
(
	const label nSmoothDisp,
	const PackedBoolList& isMasterPoint,
	const PackedBoolList& isMasterEdge,
...
bool tnbLib::snappyLayerDriver::isMaxEdge
(
...
// large feature angle
void tnbLib::snappyLayerDriver::handleFeatureAngleLayerTerminations
(
	const scalar minCos,
	const PackedBoolList& isMasterPoint,
...
void tnbLib::snappyLayerDriver::findIsolatedRegions
(
...
void tnbLib::snappyLayerDriver::medialAxisSmoothingInfo
(
	const motionSmoother& meshMover,
...
) const
{
	...
	//    wall.
	{
		...
		forAll(patches, patchi)
		{
					...
					)
			{
				...
				else
				{
					...
					scalar featureAngleCos = tnbLib::cos(degToRad(featureAngle));
					pointField pointNormals(PatchTools::pointNormals(mesh, pp));
		...
		{
			medialDist[pointi] = tnbLib::sqrt(pointMedialDist[pointi].distSqr());
	...
	forAll(medialRatio, pointi)
	{
		...
		else
		{
			...
			{
				medialRatio[pointi] = mDist / (tnbLib::sqrt(wDist2) + mDist);
			}
		}
	}
...
void tnbLib::snappyLayerDriver::shrinkMeshMedialDistance
(
