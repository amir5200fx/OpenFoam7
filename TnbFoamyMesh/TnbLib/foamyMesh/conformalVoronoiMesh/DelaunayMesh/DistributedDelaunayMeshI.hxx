...
tnbLib::DistributedDelaunayMesh<Triangulation>::buildMap
(
...
template<class Triangulation>
tnbLib::DistributedDelaunayMesh<Triangulation>::DistributedDelaunayMesh
(
	const Time& runTime
)
	:
...
template<class Triangulation>
bool tnbLib::DistributedDelaunayMesh<Triangulation>::distributeBoundBoxes
(
	const boundBox& bb
)
{
...
template<class Triangulation>
bool tnbLib::DistributedDelaunayMesh<Triangulation>::isLocal
(
...
template<class Triangulation>
tnbLib::labelList tnbLib::DistributedDelaunayMesh<Triangulation>::overlapProcessors
(
	const point& centre,
...
template<class Triangulation>
bool tnbLib::DistributedDelaunayMesh<Triangulation>::checkProcBoundaryCell
(
	const Cell_handle& cit,
	Map<labelList>& circumsphereOverlaps
) const
{
	const tnbLib::point& cc = cit->dual();
...
template<class Triangulation>
void tnbLib::DistributedDelaunayMesh<Triangulation>::findProcessorBoundaryCells
(
	Map<labelList>& circumsphereOverlaps
) const
{
...
template<class Triangulation>
void tnbLib::DistributedDelaunayMesh<Triangulation>::markVerticesToRefer
(
...
template<class Triangulation>
tnbLib::label tnbLib::DistributedDelaunayMesh<Triangulation>::referVertices
(
...
template<class Triangulation>
void tnbLib::DistributedDelaunayMesh<Triangulation>::sync
(
	const boundBox& bb,
	PtrList<labelPairHashSet>& referralVertices,
	labelPairHashSet& receivedVertices,
...
)
{
	...
	DynamicList<tnbLib::point> circumcentre(0.1 * nVerts);
...
template<class Triangulation>
tnbLib::scalar
tnbLib::DistributedDelaunayMesh<Triangulation>::calculateLoadUnbalance() const
{
...
template<class Triangulation>
bool tnbLib::DistributedDelaunayMesh<Triangulation>::distribute
(
	const boundBox& bb
)
{
...
template<class Triangulation>
tnbLib::autoPtr<tnbLib::mapDistribute>
tnbLib::DistributedDelaunayMesh<Triangulation>::distribute
(
...
template<class Triangulation>
void tnbLib::DistributedDelaunayMesh<Triangulation>::sync(const boundBox& bb)
{
	if (!Pstream::parRun())
	{
		return;
...
typename tnbLib::DistributedDelaunayMesh<Triangulation>::labelPairHashSet
tnbLib::DistributedDelaunayMesh<Triangulation>::rangeInsertReferredWithInfo
(
	PointIterator begin,
	PointIterator end,
	bool printErrors
)
{
	...
	{
		const tnbLib::point samplePoint(topoint(it->point()));

		scalar distFromBbSqr = 0;

		if (!bb.contains(samplePoint))
		{
			const tnbLib::point nearestPoint = bb.nearest(samplePoint);
