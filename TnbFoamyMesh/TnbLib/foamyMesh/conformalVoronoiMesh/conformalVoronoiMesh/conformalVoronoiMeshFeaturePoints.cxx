...
#include <ConstCirculator.hxx>
#include <DelaunayMeshTools.hxx>
...
void tnbLib::conformalVoronoiMesh::createEdgePointGroup
(
...
bool tnbLib::conformalVoronoiMesh::meshableRegion
(
...
bool tnbLib::conformalVoronoiMesh::regionIsInside
(
...
void tnbLib::conformalVoronoiMesh::createEdgePointGroupByCirculating
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
...
{
    typedef tnbLib::indexedVertexEnum::vertexType         vertexType;
    typedef extendedFeatureEdgeMesh::sideVolumeType     sideVolumeType;

    const tnbLib::point& edgePt = edHit.hitPoint();
    ...
    if (circ.size()) do
    {
        ...
        const tnbLib::point masterPt = edgePt + ppDist * masterPtVec;
        ...
        if (mag((normalDir & nextNormalDir) - 1) < small)
        {
            if (inside)
            {
                ...
                tnbLib::point pt1 = edgePt + s + ppDist * normal;
                tnbLib::point pt2 = edgePt - s + ppDist * normal;

                tnbLib::point pt3 = facePlane.mirror(pt1);
                tnbLib::point pt4 = facePlane.mirror(pt2);
    ...
    forAllConstIter(Map<tnbLib::point>, masterPoints, iter)
    {
        const tnbLib::point& pt = masterPoints[iter.key()];
        ...
        {
            const tnbLib::point reflectedPt =
...
void tnbLib::conformalVoronoiMesh::createExternalEdgePointGroup
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
) const
{
    ...
    tnbLib::point refPt = edgePt - ppDist * refVec;
    ...
    tnbLib::point reflectedA = refPt + 2 * ppDist * nA;
    pts.append
    (
        Vb
        (
    ...
    tnbLib::point reflectedB = refPt + 2 * ppDist * nB;
    pts.append
    (
        Vb
        (
...
void tnbLib::conformalVoronoiMesh::createInternalEdgePointGroup
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
) const
{
    ...
    tnbLib::point reflMasterPt = refPt + 2 * (edgePt - refPt);

    // Reflect reflMasterPt in both faces.
    tnbLib::point reflectedA = reflMasterPt - 2 * ppDist * nA;

    tnbLib::point reflectedB = reflMasterPt - 2 * ppDist * nB;
    ...
    {
        tnbLib::point reflectedAa = refPt + ppDist * nB;
        pts.append
        (
            Vb
            (
        ...
        tnbLib::point reflectedBb = refPt + ppDist * nA;
        pts.append
        (
            Vb
            (
...
void tnbLib::conformalVoronoiMesh::createFlatEdgePointGroup
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
...
void tnbLib::conformalVoronoiMesh::createOpenEdgePointGroup
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
...
void tnbLib::conformalVoronoiMesh::createMultipleEdgePointGroup
(
    const extendedFeatureEdgeMesh& feMesh,
    const pointIndexHit& edHit,
    DynamicList<Vb>& pts
) const
{
    ...
    if (nNormalTypes[extendedFeatureEdgeMesh::BOTH] == 4)
    {
        ...
        tnbLib::point pt1 = edgePt + ppDist * normalDir + ppDist * n;
        tnbLib::point pt2 = edgePt + ppDist * normalDir - ppDist * n;

        plane plane3(edgePt, normalDir);

        tnbLib::point pt3 = plane3.mirror(pt1);
        tnbLib::point pt4 = plane3.mirror(pt2);
...
void tnbLib::conformalVoronoiMesh::insertFeaturePoints(bool distribute)
{
    Info << nl
...
//tnbLib::scalar tnbLib::conformalVoronoiMesh::pyramidVolume
//(
//    const tnbLib::point& apex,
//    const tnbLib::point& a,
//    const tnbLib::point& b,
//    const tnbLib::point& c,
...
//void tnbLib::conformalVoronoiMesh::createPyramidMasterPoint
//(
//    const tnbLib::point& apex,
//    const vectorField& edgeDirections,
//    tnbLib::point& masterPoint,
//    vectorField& norms
//) const
//{
...
//void tnbLib::conformalVoronoiMesh::createConvexConcaveFeaturePoints
//(
...
//        )
//        {
//            const tnbLib::point& apex = feMesh.points()[ptI];
//
