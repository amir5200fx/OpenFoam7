...
#include <indexedCellChecks.hxx>
#include <IOmanip.hxx>
#include <OBJstream.hxx>

using namespace tnbLib::vectorTools;

const tnbLib::scalar tnbLib::conformalVoronoiMesh::searchConeAngle
= tnbLib::cos(degToRad(30));

const tnbLib::scalar tnbLib::conformalVoronoiMesh::searchAngleOppositeSurface
= tnbLib::cos(degToRad(150));


// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

void tnbLib::conformalVoronoiMesh::conformToSurface()
{
    this->resetCellCount();
    // Index the cells
    for
...
bool tnbLib::conformalVoronoiMesh::reconformToSurface() const
{
    if
        (
            runTime_.timeIndex()
...
tnbLib::label tnbLib::conformalVoronoiMesh::findVerticesNearBoundaries()
{
...
void tnbLib::conformalVoronoiMesh::buildSurfaceConformation()
{
    ...
    const label AtoV = label(6 / tnbLib::pow(scalar(number_of_vertices()), 3));
...
tnbLib::label tnbLib::conformalVoronoiMesh::synchroniseSurfaceTrees
(
...
)
{
    ...
    for (label proci = 0; proci < Pstream::nProcs(); ++proci)
    {
        ...
        {
            const tnbLib::point& pt = otherSurfEdges[peI].first().hitPoint();
...
tnbLib::label tnbLib::conformalVoronoiMesh::synchroniseEdgeTrees
(
...
)
{
    ...
    for (label proci = 0; proci < Pstream::nProcs(); ++proci)
    {
        ...
        {
            const tnbLib::point& pt = otherProcEdges[peI].first().hitPoint();
...
bool tnbLib::conformalVoronoiMesh::surfaceLocationConformsToInside
(
...
) const
{
    if (info.first().hit())
    {
        ...
        const tnbLib::point innerPoint = info.first().hitPoint() - ppDist * n;
...
bool tnbLib::conformalVoronoiMesh::dualCellSurfaceAnyIntersection
(
...
) const
{
            ...
            )
    {
        ...
        tnbLib::point dE0 = fit->first->dual();
        tnbLib::point dE1 = fit->first->neighbor(fit->second)->dual();
...
bool tnbLib::conformalVoronoiMesh::dualCellSurfaceAllIntersections
(
    const Delaunay::Finite_vertices_iterator& vit,
...
) const
{
            ...
            )
    {
        ...
        if (infoIntersection.hit())
        {
            ...
            tnbLib::point newPoint = vertex + d * n;
...
bool tnbLib::conformalVoronoiMesh::clipLineToProc
(
...
void tnbLib::conformalVoronoiMesh::dualCellLargestSurfaceProtrusion
(
    const Delaunay::Finite_vertices_iterator& vit,
...
) const
{
            ...
            )
    {
        ...
        //        tnbLib::point endPt = 0.5*(c1->dual() + c2->dual());
        tnbLib::point endPt = c1->dual();
        ...
        if (surfHit.hit())
        {
            ...
            if (normalProtrusionDistance > maxProtrusionDistance)
            {
                ...
                tnbLib::point newPoint = endPt - d * n;
...
void tnbLib::conformalVoronoiMesh::dualCellLargestSurfaceIncursion
(
    const Delaunay::Finite_vertices_iterator& vit,
    pointIndexHit& surfHitLargest,
    label& hitSurfaceLargest
...
void tnbLib::conformalVoronoiMesh::reportProcessorOccupancy()
{
    for
        (
            Delaunay::Finite_vertices_iterator vit = finite_vertices_begin();
...
//void tnbLib::conformalVoronoiMesh::reportSurfaceConformationQuality()
//{
...
//        {
//            tnbLib::point vert(topoint(vit->point()));
//            pointIndexHit surfHit;
//            label hitSurface;
//
//            dualCellLargestSurfaceProtrusion(vit, surfHit, hitSurface);
...
//
//                tnbLib::point pt = topoint(vit->point());
//
...
void tnbLib::conformalVoronoiMesh::limitDisplacement
(
    const Delaunay::Finite_vertices_iterator& vit,
    vector& displacement,
    label callCount
) const
{
    ...
    pointFromPoint pt = topoint(vit->point());
    tnbLib::point dispPt = pt + displacement;
...
tnbLib::scalar tnbLib::conformalVoronoiMesh::angleBetweenSurfacePoints
(
    tnbLib::point pA,
    tnbLib::point pB
) const
...
bool tnbLib::conformalVoronoiMesh::nearSurfacePoint
(
    pointIndexHitAndFeature& pHit
) const
{
    const tnbLib::point& pt = pHit.first().hitPoint();
...
bool tnbLib::conformalVoronoiMesh::appendToSurfacePtTree
(
    const tnbLib::point& pt
) const
{
...
bool tnbLib::conformalVoronoiMesh::appendToEdgeLocationTree
(
    const tnbLib::point& pt
) const
{
...
tnbLib::List<tnbLib::pointIndexHit>
tnbLib::conformalVoronoiMesh::nearestFeatureEdgeLocations
(
    const tnbLib::point& pt
) const
{
    ...
    return tnbLib::move(dynPointHit);
}


bool tnbLib::conformalVoronoiMesh::pointIsNearFeatureEdgeLocation
(
    const tnbLib::point& pt
) const
{
...
bool tnbLib::conformalVoronoiMesh::pointIsNearSurfaceLocation
(
    const tnbLib::point& pt
) const
{
...
bool tnbLib::conformalVoronoiMesh::nearFeatureEdgeLocation
(
    const pointIndexHit& pHit,
    pointIndexHit& nearestEdgeHit
) const
{
    const tnbLib::point& pt = pHit.hitPoint();
...
void tnbLib::conformalVoronoiMesh::buildEdgeLocationTree
(
...
void tnbLib::conformalVoronoiMesh::buildSurfacePtLocationTree
(
...
void tnbLib::conformalVoronoiMesh::addSurfaceAndEdgeHits
(
...
) const
{
    ...
    forAll(surfaceIntersections, sI)
    {
        ...
        const tnbLib::point& surfPt = surfHitI.first().hitPoint();
        ...
        forAll(edHitsByFeature, i)
        {
            ...
            forAll(edHits, eHitI)
            {
                ...
                if (edHit.hit())
                {
                    const tnbLib::point& edPt = edHit.hitPoint();
...
void tnbLib::conformalVoronoiMesh::storeSurfaceConformation()
{
...
void tnbLib::conformalVoronoiMesh::reinsertSurfaceConformation()
{
