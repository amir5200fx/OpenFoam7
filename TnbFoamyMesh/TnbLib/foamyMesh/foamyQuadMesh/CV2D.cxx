...
void tnbLib::CV2D::insertBoundingBox()
{
...
void tnbLib::CV2D::fast_restore_Delaunay(Vertex_handle vh)
{
    int i;
...
void tnbLib::CV2D::external_flip(Face_handle& f, int i)
{
...
bool tnbLib::CV2D::internal_flip(Face_handle& f, int i)
{
    Face_handle n = f->neighbor(i);
...
void tnbLib::CV2D::insertPoints(const fileName& pointFileName)
{
...
void tnbLib::CV2D::insertSurfacePointPairs()
{
...
void tnbLib::CV2D::boundaryConform()
{
    if (!meshControls().insertSurfaceNearestPointPairs())
    {
    ...
    for (label iter = 1; iter <= meshControls().maxBoundaryConformingIter(); iter++)
    {
        ...
        (
            "surfaceIntersections_" + tnbLib::name(iter) + ".obj"
        );
...
void tnbLib::CV2D::removeSurfacePointPairs()
{
    for
        (
...
/*
void tnbLib::CV2D::moveInternalPoints(const point2DField& newPoints)
{
    label pointi = 0;
...
void tnbLib::CV2D::write(const word& stage) const
{
    if (meshControls().objOutput())
    {
        tnbLib::mkDir(stage + "Faces");
        tnbLib::mkDir(stage + "Triangles");
