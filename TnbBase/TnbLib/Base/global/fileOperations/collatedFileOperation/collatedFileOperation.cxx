...
tnbLib::labelList tnbLib::fileOperations::collatedFileOperation::ioRanks()
{
	labelList ioRanks;
...
bool tnbLib::fileOperations::collatedFileOperation::isMasterRank
(
	const label proci
)
const
...
bool tnbLib::fileOperations::collatedFileOperation::appendObject
(
...
tnbLib::fileOperations::collatedFileOperation::collatedFileOperation
(
	const bool verbose
)
	:
...
tnbLib::fileName tnbLib::fileOperations::collatedFileOperation::objectPath
(
	const IOobject& io,
	const word& typeName
) const
...
bool tnbLib::fileOperations::collatedFileOperation::writeObject
(
	const regIOobject& io,
	IOstream::streamFormat fmt,
	IOstream::versionNumber ver,
...
void tnbLib::fileOperations::collatedFileOperation::flush() const
{
	if (debug)
	{
...
tnbLib::word tnbLib::fileOperations::collatedFileOperation::processorsDir
(
...
) const
{
	if (Pstream::parRun())
	{
		...
		word procDir(processorsBaseDir + tnbLib::name(Pstream::nProcs()));
		...
		if (procs.size() != Pstream::nProcs())
		{
			procDir +=
				...
				+ tnbLib::name(procs[0])
				+ "-"
				+ tnbLib::name(procs.last());
		}
		return procDir;
	}
	else
	{
		word procDir(processorsBaseDir + tnbLib::name(nProcs_));
...
void tnbLib::fileOperations::collatedFileOperation::setNProcs(const label nProcs)
{
	nProcs_ = nProcs;
