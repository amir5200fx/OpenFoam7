...
tnbLib::HashTable<tnbLib::string> tnbLib::argList::optionUsage;
tnbLib::SLList<tnbLib::string>    tnbLib::argList::notes;
tnbLib::string::size_type tnbLib::argList::usageMin = 20;
tnbLib::string::size_type tnbLib::argList::usageMax = 80;
tnbLib::word tnbLib::argList::postProcessOptionName("postProcess");
...
void tnbLib::argList::initValidTables::clear()
{
...
void tnbLib::argList::addBoolOption
(
	const word& opt,
	const string& usage
)
...
void tnbLib::argList::addNote(const string& note)
{
	if (!note.empty())
	{
		notes.append(note);
...
void tnbLib::argList::removeOption(const word& opt)
{
...
void tnbLib::argList::printOptionUsage
(
...
bool tnbLib::argList::postProcess(int argc, char *argv[])
{
...
)
{
	...
	if (initialise)
	{
		...
		{
			std::string build(tnbLib::FOAMbuild);
	...
	if (parRunControl_.parRun())
	{
		if (Pstream::master())
		{
					...
					)
			{
				...
				// the same build
				if (slaveBuild != tnbLib::FOAMbuild)
				{
					FatalErrorIn(executable())
						<< "Master is running version " << tnbLib::FOAMbuild
		...
		else
		{
			...
			toMaster << string(tnbLib::FOAMbuild) << hostName() << pid();
		}
	}
...
bool tnbLib::argList::setOption(const word& opt, const string& param)
{
	bool changed = false;
...
bool tnbLib::argList::unsetOption(const word& opt)
{
	// Only allow valid options
	if (validOptions.found(opt))
	{
...
void tnbLib::argList::printNotes() const
{
...
void tnbLib::argList::printUsage() const
{
	...
	Info << nl
		<< "Using: OpenFOAM-" << tnbLib::FOAMversion
