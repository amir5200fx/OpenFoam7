...
tnbLib::dimensionSet::tokeniser::tokeniser(Istream& is)
	:
	is_(is),
	tokens_(100),
	start_(0),
...
void tnbLib::dimensionSet::tokeniser::push(const token& t)
{
...
tnbLib::token tnbLib::dimensionSet::tokeniser::pop()
{
...
void tnbLib::dimensionSet::tokeniser::unpop(const token& t)
{
	++size_;
...
bool tnbLib::dimensionSet::tokeniser::hasToken() const
{
	return size_ || is_.good();
}


bool tnbLib::dimensionSet::tokeniser::valid(char c)
{
	return
		(
			!isspace(c)
...
tnbLib::label tnbLib::dimensionSet::tokeniser::priority(const token& t)
{
	if (!t.isPunctuation())
	{
		return 0;
...
void tnbLib::dimensionSet::tokeniser::splitWord(const word& w)
{
	size_t start = 0;
...
tnbLib::token tnbLib::dimensionSet::tokeniser::nextToken()
{
	if (size_ == 0)
	{
		token t(is_);
...
void tnbLib::dimensionSet::tokeniser::putBack(const token& t)
{
	if (size_ == 0)
	{
		push(t);
...
void tnbLib::dimensionSet::round(const scalar tol)
{
...
tnbLib::dimensionedScalar tnbLib::dimensionSet::parse
(
		...
		else if (nextToken.isPunctuation())
		{
			...
			else if (nextToken.pToken() == '^')
			{
				if (nextPrior > lastPrior)
				{
					...
					ds.value() = tnbLib::pow(ds.value(), exp.value());
				}
				else
				{
					tis.putBack(nextToken);
...
tnbLib::Istream& tnbLib::dimensionSet::read
(
	Istream& is,
	scalar& multiplier,
	const HashTable<dimensionedScalar>& readSet
	...
	if (nextToken.isWord())
	{
		...
		do
		{
			...
			if (index != string::npos)
			{
				...
				multiplier *= tnbLib::pow(s.value(), exponent);
			}
			else
			{
				dimensionedScalar s;
...
tnbLib::Ostream& tnbLib::dimensionSet::write
(
	Ostream& os,
	scalar& multiplier,
		...
		forAll(exponents, i)
		{
			if (mag(exponents[i]) > smallExponent)
			{
				...
				if (mag(exponents[i] - 1) > smallExponent)
				{
					...
					multiplier *= tnbLib::pow(ds.value(), exponents[i]);
				}
				else
				{
					multiplier *= ds.value();
...
void tnbLib::writeEntry(Ostream& os, const dimensionSet& value)
{
	os << value;
}
