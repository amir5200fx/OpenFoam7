...
namespace tnbLib
{
	...
	mode_t mode(const fileName& name, const bool /*checkVariants*/, const bool /*followLink*/)
	{
	...
	fileType type(const fileName& name, const bool /*checkVariants*/, const bool /*followLink*/)
	{
		...
		{
			t = (attrs & FILE_ATTRIBUTE_DIRECTORY) ?
				fileType::directory :
				fileType::file;
		}
	...
	bool exists(const fileName& name, const bool checkGzip, const bool /*followLink*/)
	{
		const DWORD attrs = ::GetFileAttributes(name.c_str());
		const bool success = (attrs != INVALID_FILE_ATTRIBUTES) ||
	...
	// Does the file exist
	bool isFile(const fileName& name, const bool checkGzip, const bool /*followLink*/)
	{
		const DWORD attrs = ::GetFileAttributes(name.c_str());
		const bool success = ((attrs != INVALID_FILE_ATTRIBUTES) &&
			!(attrs & FILE_ATTRIBUTE_DIRECTORY)) ||
	...
	// Return size of file
	off_t fileSize(const fileName& name, const bool /*checkVariants*/, const bool /*followLink*/)
	{
		fileStat fileStatus(name);
	...
	time_t lastModified(const fileName& name, const bool /*checkVariants*/, const bool /*followLink*/)
	{
		fileStat fileStatus(name);
	...
	)
	{
		...
		if (fileStatus.isValid())
		{
			...
			return
				fileStatus.status().st_mtime
				+ 1e-9*fileStatus.status().st_atime;
		}
		else
		{
			return 0;
