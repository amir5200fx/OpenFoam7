...
void tnbLib::UPstream::addValidParOptions(HashTable<string>& validParOptions)
{
...
bool tnbLib::UPstream::init(int& argc, char**& argv, const bool needsThread)
{
	// MPI_Init(&argc, &argv);
...
void tnbLib::UPstream::exit(int errnum)
{
	if (debug)
	{
...
void tnbLib::UPstream::allocatePstreamCommunicator
(
	...
	if (parentIndex == -1)
	{
		...
		if (index != UPstream::worldComm)
		{
			FatalErrorInFunction
				...
				<< UPstream::worldComm << tnbLib::exit(FatalError);
		}
...
void tnbLib::UPstream::freePstreamCommunicator(const label communicator)
{
...
tnbLib::label tnbLib::UPstream::nRequests()
{
	return PstreamGlobals::outstandingRequests_.size();
}


void tnbLib::UPstream::resetRequests(const label i)
{
...
void tnbLib::UPstream::waitRequests(const label start)
{
	if (debug)
	{
...
void tnbLib::UPstream::waitRequest(const label i)
{
	if (debug)
	{
	...
	{
		FatalErrorInFunction
			<< "MPI_Wait returned with error" << tnbLib::endl;
	}
...
bool tnbLib::UPstream::finishedRequest(const label i)
{
	if (debug)
	{
...
int tnbLib::UPstream::allocateTag(const char* s)
{
	int tag;
...
int tnbLib::UPstream::allocateTag(const word& s)
{
	int tag;
	if (PstreamGlobals::freedTags_.size())
	{
...
void tnbLib::UPstream::freeTag(const char* s, const int tag)
{
	if (debug)
	{
		// if (UPstream::lateBlocking > 0)
...
void tnbLib::UPstream::freeTag(const word& s, const int tag)
{
	if (debug)
	{
		// if (UPstream::lateBlocking > 0)
