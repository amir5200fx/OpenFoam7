...
namespace tnbLib
{
	...
	public:
		...
		// Constructors
			...
			//- Construct Null
		FoamBase_EXPORT scalarRanges();

		//- Construct from Istream.
		//  The list items are comma-delimited.
		FoamBase_EXPORT scalarRanges(Istream&);
		...
		FoamBase_EXPORT List<bool> selected(const List<scalar>&) const;

		//- Select a list of values that are within the ranges
		FoamBase_EXPORT List<scalar> select(const List<scalar>&) const;

		//- Select a list of values that are within the ranges
		FoamBase_EXPORT void inplaceSelect(List<scalar>&) const;
	};
