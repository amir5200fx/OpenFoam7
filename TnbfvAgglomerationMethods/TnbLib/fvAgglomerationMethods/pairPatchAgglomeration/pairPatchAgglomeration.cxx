...
void tnbLib::pairPatchAgglomeration::compactLevels(const label nCreatedLevels)
{
...
bool tnbLib::pairPatchAgglomeration::continueAgglomerating
(
...
void tnbLib::pairPatchAgglomeration::setBasedEdgeWeights()
{
    ...
    forAll(coarsePatch.edges(), i)
    {
        if (coarsePatch.isInternalEdge(i))
        {
            ...
            if (eFaces.size() == 2)
            {
                ...
                if (cosI < tnbLib::cos(degToRad(featureAngle_)))
                {
...
void tnbLib::pairPatchAgglomeration::setEdgeWeights
(
...
tnbLib::pairPatchAgglomeration::pairPatchAgglomeration
(
...
const tnbLib::bPatch& tnbLib::pairPatchAgglomeration::patchLevel
(
    const label i
) const
{
...
void tnbLib::pairPatchAgglomeration::mapBaseToTopAgglom
(
    const label fineLevelIndex
)
{
...
bool tnbLib::pairPatchAgglomeration::agglomeratePatch
(
    const bPatch& patch,
...
void tnbLib::pairPatchAgglomeration::agglomerate()
{
    label nPairLevels = 0;
...
tnbLib::tmp<tnbLib::labelField> tnbLib::pairPatchAgglomeration::agglomerateOneLevel
(
    label& nCoarseFaces,
    const bPatch& patch
)
...
void tnbLib::pairPatchAgglomeration::combineLevels(const label curLevel)
{
