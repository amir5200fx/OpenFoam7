...
bool tnbLib::meshCutter::uses(const labelList& elems1, const labelList& elems2)
{
    forAll(elems1, elemI)
    {
...
tnbLib::label tnbLib::meshCutter::findCutCell
(
    const cellCuts& cuts,
...
tnbLib::label tnbLib::meshCutter::findInternalFacePoint
(
...
void tnbLib::meshCutter::faceCells
(
    const cellCuts& cuts,
    const label facei,
    label& own,
...
void tnbLib::meshCutter::getFaceInfo
(
    const label facei,
    label& patchID,
    label& zoneID,
...
void tnbLib::meshCutter::addFace
(
...
void tnbLib::meshCutter::modFace
(
    polyTopoChange& meshMod,
    const label facei,
    const face& newFace,
...
void tnbLib::meshCutter::copyFace
(
    const face& f,
    const label startFp,
    const label endFp,
...
void tnbLib::meshCutter::splitFace
(
    const face& f,
    const label v0,
    const label v1,
...
tnbLib::face tnbLib::meshCutter::addEdgeCutsToFace(const label facei) const
{
    const face& f = mesh().faces()[facei];
...
tnbLib::face tnbLib::meshCutter::loopToFace
(
    const label celli,
...
tnbLib::meshCutter::meshCutter(const polyMesh& mesh)
    :
    edgeVertex(mesh),
    addedCells_(),
    addedFaces_(),
...
tnbLib::meshCutter::~meshCutter()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void tnbLib::meshCutter::setRefinement
(
    const cellCuts& cuts,
    polyTopoChange& meshMod
)
...
void tnbLib::meshCutter::updateMesh(const mapPolyMesh& morphMap)
{
