...
tnbLib::HashSet<tnbLib::label> tnbLib::edgeCollapser::checkBadFaces
(
	const polyMesh& mesh,
...
tnbLib::label tnbLib::edgeCollapser::checkMeshQuality
(
	const polyMesh& mesh,
	const dictionary& meshQualityDict,
...
tnbLib::labelList tnbLib::edgeCollapser::edgesFromPoints
(
	const label& facei,
...
void tnbLib::edgeCollapser::collapseToEdge
(
	const label facei,
...
) const
{
	...
	tnbLib::point collapseToPtA(great, great, great);
	...
	// Positive half
	tnbLib::point collapseToPtB(great, great, great);
...
void tnbLib::edgeCollapser::collapseToPoint
(
	const label& facei,
	const pointField& pts,
	const labelList& pointPriority,
...
void tnbLib::edgeCollapser::faceCollapseAxisAndAspectRatio
(
	const face& f,
	const point& fC,
	vector& collapseAxis,
...
) const
{
	...
	if (detJ < 1e-5)
	{
		...
		aspectRatio = tnbLib::sqrt(0.35 / detJ);
	}
	else
	{
		...
		else
		{
			...
			aspectRatio = tnbLib::sqrt(eVals.y() / max(eVals.x(), small));
		}
	}
}


tnbLib::scalarField tnbLib::edgeCollapser::calcTargetFaceSizes() const
{
	...
	for (label intFacei = 0; intFacei < mesh_.nInternalFaces(); ++intFacei)
	{
		...
		scalar targetFaceSizeA = tnbLib::pow(cellOwnerVol, 1.0 / 3.0);
		scalar targetFaceSizeB = tnbLib::pow(cellNeighbourVol, 1.0 / 3.0);
	...
	forAll(patches, patchi)
	{
		...
		else
		{
			...
			forAll(patch, patchFacei)
			{
				...
				targetFaceSizes[extFacei] = tnbLib::pow(cellOwnerVol, 1.0 / 3.0);
			}
		}
	}
...
tnbLib::edgeCollapser::collapseType tnbLib::edgeCollapser::collapseFace
(
	const labelList& pointPriority,
	const face& f,
	const label facei,
...
) const
{
	...
	const tnbLib::point fC = f.centre(pts);
	...
	forAll(f, fPtI)
	{
		const tnbLib::point& pt = pts[f[fPtI]];
...
tnbLib::label tnbLib::edgeCollapser::edgeMaster
(
	const labelList& pointPriority,
	const edge& e
) const
...
void tnbLib::edgeCollapser::checkBoundaryPointMergeEdges
(
	const label pointi,
...
tnbLib::label tnbLib::edgeCollapser::breakStringsAtEdges
(
	PackedBoolList& collapseEdge,
...
void tnbLib::edgeCollapser::determineDuplicatePointsOnFace
(
	const face& f,
...
tnbLib::label tnbLib::edgeCollapser::countEdgesOnFace
(
	const face& f,
	List<pointEdgeCollapse>& allPointInfo
) const
...
bool tnbLib::edgeCollapser::isFaceCollapsed
(
	const face& f,
	List<pointEdgeCollapse>& allPointInfo
) const
...
tnbLib::label tnbLib::edgeCollapser::syncCollapse
(
...
void tnbLib::edgeCollapser::filterFace
(
...
tnbLib::edgeCollapser::edgeCollapser(const polyMesh& mesh)
	:
	mesh_(mesh),
	guardFraction_(0),
...
tnbLib::edgeCollapser::edgeCollapser
(
	const polyMesh& mesh,
...
bool tnbLib::edgeCollapser::setRefinement
(
	const List<pointEdgeCollapse>& allPointInfo,
...
void tnbLib::edgeCollapser::consistentCollapse
(
	const globalIndex& globalPoints,
	const labelList& pointPriority,
	const Map<point>& collapsePointToLocation,
...
tnbLib::label tnbLib::edgeCollapser::markSmallEdges
(
...
tnbLib::label tnbLib::edgeCollapser::markMergeEdges
(
	const scalar maxCos,
	const labelList& pointPriority,
	PackedBoolList& collapseEdge,
...
tnbLib::labelPair tnbLib::edgeCollapser::markSmallSliverFaces
(
	const scalarField& faceFilterFactor,
	const labelList& pointPriority,
	PackedBoolList& collapseEdge,
...
tnbLib::labelPair tnbLib::edgeCollapser::markFaceZoneEdges
(
...
) const
{
	...
	//
	//            const tnbLib::point collapsePoint =
