...
void tnbLib::hexRef8::getFaceInfo
(
	const label facei,
	label& patchID,
	label& zoneID,
...
tnbLib::label tnbLib::hexRef8::addFace
(
...
// for the face)
tnbLib::label tnbLib::hexRef8::addInternalFace
(
	polyTopoChange& meshMod,
...
tnbLib::scalar tnbLib::hexRef8::getLevel0EdgeLength() const
{
	...
	forAll(typEdgeLenSqr, levelI)
	{
		...
		if (lenSqr < great2)
		{
			level0Size = tnbLib::sqrt(lenSqr)*(1 << levelI);

			if (debug)
			{
				Pout << "hexRef8::getLevel0EdgeLength() :"
					<< " For level:" << levelI
					<< " have edgeLen:" << tnbLib::sqrt(lenSqr)
...
tnbLib::label tnbLib::hexRef8::getAnchorCell
(
...
void tnbLib::hexRef8::getFaceNeighbours
(
	const labelListList& cellAnchorPoints,
	const labelListList& cellAddedCells,
	const label facei,
...
tnbLib::label tnbLib::hexRef8::findMinLevel(const labelList& f) const
{
...
tnbLib::label tnbLib::hexRef8::findMaxLevel(const labelList& f) const
{
...
tnbLib::label tnbLib::hexRef8::countAnchors
(
	const labelList& f,
...
void tnbLib::hexRef8::dumpCell(const label celli) const
{
	OFstream str(mesh_.time().path() / "cell_" + tnbLib::name(celli) + ".obj");
...
tnbLib::label tnbLib::hexRef8::findLevel
(
	const label facei,
	const face& f,
	const label startFp,
...
tnbLib::label tnbLib::hexRef8::faceLevel(const label facei) const
{
	const face& f = mesh_.faces()[facei];
...
void tnbLib::hexRef8::checkInternalOrientation
(
	polyTopoChange& meshMod,
	const label celli,
	const label facei,
...
void tnbLib::hexRef8::checkBoundaryOrientation
(
	polyTopoChange& meshMod,
	const label celli,
	const label facei,
...
// splitPoint.
void tnbLib::hexRef8::insertEdgeSplit
(
...
tnbLib::label tnbLib::hexRef8::storeMidPointInfo
(
	const labelListList& cellAnchorPoints,
	const labelListList& cellAddedCells,
...
void tnbLib::hexRef8::createInternalFaces
(
	const labelListList& cellAnchorPoints,
	const labelListList& cellAddedCells,
	const labelList& cellMidPoint,
...
void tnbLib::hexRef8::walkFaceToMid
(
	const labelList& edgeMidPoint,
	const label cLevel,
	const label facei,
...
void tnbLib::hexRef8::walkFaceFromMid
(
	const labelList& edgeMidPoint,
	const label cLevel,
	const label facei,
...
tnbLib::label tnbLib::hexRef8::faceConsistentRefinement
(
	const bool maxSet,
...
void tnbLib::hexRef8::checkWantedRefinementLevels
(
...
void tnbLib::hexRef8::setInstance(const fileName& inst)
{
	if (debug)
	{
...
void tnbLib::hexRef8::collectLevelPoints
(
	const labelList& f,
	const label level,
...
bool tnbLib::hexRef8::matchHexShape
(
	const label celli,
...
tnbLib::hexRef8::hexRef8(const polyMesh& mesh, const bool readHistory)
	:
	mesh_(mesh),
	cellLevel_
	(
...
tnbLib::labelList tnbLib::hexRef8::consistentRefinement
(
	const labelList& cellsToRefine,
	const bool maxSet
) const
...
tnbLib::labelList tnbLib::hexRef8::consistentSlowRefinement
(
...
tnbLib::labelList tnbLib::hexRef8::consistentSlowRefinement2
(
	const label maxFaceDiff,
	const labelList& cellsToRefine,
	const labelList& facesToCheck
...
tnbLib::labelListList tnbLib::hexRef8::setRefinement
(
...
void tnbLib::hexRef8::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void tnbLib::hexRef8::updateMesh
(
...
void tnbLib::hexRef8::distribute(const mapDistributePolyMesh& map)
{
	if (debug)
	{
...
void tnbLib::hexRef8::checkMesh() const
{
...
void tnbLib::hexRef8::checkRefinementLevels
(
	const label maxPointDiff,
	const labelList& pointsToCheck
) const
...
const tnbLib::cellShapeList& tnbLib::hexRef8::cellShapes() const
{
...
tnbLib::labelList tnbLib::hexRef8::getSplitPoints() const
{
	if (debug)
	{
		checkRefinementLevels(-1, labelList(0));
...
//void tnbLib::hexRef8::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
tnbLib::labelList tnbLib::hexRef8::consistentUnrefinement
(
...
void tnbLib::hexRef8::setUnrefinement
(
...
bool tnbLib::hexRef8::write(const bool write) const
{
	bool writeOk =
