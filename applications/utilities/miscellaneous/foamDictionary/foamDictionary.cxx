...
Usage
    ...
    Example usage:
      ...
      - Change patch type:
        ...
        in the dictionary (after ignoring the tnbLibFile entry).
...
IOstream::streamFormat readDict(dictionary& dict, const fileName& dictFileName)
{
    ...
    // If the first entry is the "tnbLibFile" header dictionary
    // read and set the stream format
    if (firstEntry->isDict() && firstEntry->keyword() == "tnbLibFile")
    {
    ...
    // preserving the "tnbLibFile" header dictionary if present
...
#include <removeCaseOptions.lxx>
    ...
    if (listIncludes)
    {
        tnbLib::functionEntries::includeEntry::log = true;
    }
