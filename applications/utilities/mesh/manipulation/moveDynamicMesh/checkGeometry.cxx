...
tnbLib::label tnbLib::findOppositeWedge
(
    const polyMesh& mesh,
...
bool tnbLib::checkCoupledPoints
(
    const polyMesh& mesh,
    const bool report,
    labelHashSet* setPtr
...
void tnbLib::writeAMIWeightsSum
(
    const polyMesh& mesh,
...
)
{
    ...
    tnbLib::PatchTools::gatherAndMerge
    (
        mesh,
        patch.localFaces(),
        patch.meshPoints(),
...
void tnbLib::writeAMIWeightsSums(const polyMesh& mesh)
{
...
tnbLib::label tnbLib::checkGeometry
(
    const polyMesh& mesh,
    ...
    if (allGeometry)
    {
        ...
        if (mesh.checkPointNearness(false, minDistSqr, &points))
        {
            ...
            if (nPoints > nEdgeClose)
            {
                ...
                Info << "  <<Writing " << nPoints
                    << " near (closer than " << tnbLib::sqrt(minDistSqr)
