...
#include "domainDecomposition.hxx"
...
#include <sphericalTensorFieldIOField.hxx>
...
#include "dimFieldDecomposer.hxx"
#include <fvFieldDecomposer.hxx>
#include "pointFieldDecomposer.hxx"
...
int main(int argc, char *argv[])
{
    ...
    #include <addAllRegionsOption.lxx>
        ...
        if (forceOverwrite)
        {
            ...
            forAllReverse(dirs, diri)
            {
                ...
                if (d.find("processor") == 0)
                {
                    ...
                    label proci = -1;
                    if (tnbLib::read(num.c_str(), proci))
                    {
                        if (fileHandler().exists(d))
                        {
                            fileHandler().rmDir(d);
    ...
    forAll(regionNames, regioni)
    {
        ...
        const word& regionDir = tnbLib::regionDir(regionName);
        ...
        (
            ...
            "" // changed from "dictIO.objectPath()" to "" by Amin
        );
