...
const tnbLib::scalar coeff[] = { 0.11, 0.45, -0.94, 1.58, -2.58, 0.08, 3.15, -4.78 };
const char* polyDef = "(0.11 0.45 -0.94 1.58 -2.58 0.08 3.15 -4.78)";


tnbLib::scalar polyValue(const tnbLib::scalar x)
{
	// Hard-coded polynomial 8 coeff (7th order)
	return
		coeff[0]
		+ coeff[1] * x
		+ coeff[2] * tnbLib::sqr(x)
		+ coeff[3] * tnbLib::pow3(x)
		+ coeff[4] * tnbLib::pow4(x)
		+ coeff[5] * tnbLib::pow5(x)
		+ coeff[6] * tnbLib::pow6(x)
		+ coeff[7] * x * tnbLib::pow6(x);
}


tnbLib::scalar intPolyValue(const tnbLib::scalar x)
{
	// Hard-coded integral form of above polynomial
	return
		coeff[0] * x
		+ coeff[1] / 2.0 * tnbLib::sqr(x)
		+ coeff[2] / 3.0 * tnbLib::pow3(x)
		+ coeff[3] / 4.0 * tnbLib::pow4(x)
		+ coeff[4] / 5.0 * tnbLib::pow5(x)
		+ coeff[5] / 6.0 * tnbLib::pow6(x)
		+ coeff[6] / 7.0 * x * tnbLib::pow6(x)
		+ coeff[7] / 8.0 * x * x * tnbLib::pow6(x);
}


tnbLib::scalar polyValue1(const tnbLib::scalar x)
{
	// "normal" evaluation using pow()
	tnbLib::scalar value = coeff[0];
...
tnbLib::scalar polyValue2(const tnbLib::scalar x)
{
	// calculation avoiding pow()
	tnbLib::scalar value = coeff[0];

	tnbLib::scalar powX = x;
	for (int i = 1; i < PolySize; ++i)
	{
		value += coeff[i] * powX;
		powX *= x;
