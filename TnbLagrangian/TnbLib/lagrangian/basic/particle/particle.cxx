...
#include <mapPolyMesh.hxx>  // added by amir
#include <meshTools.hxx>  // added by amir
#include <indexedOctree.hxx>  // added by amir

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

const tnbLib::label tnbLib::particle::maxNBehind_ = 10;

tnbLib::label tnbLib::particle::particleCount_ = 0;
...
void tnbLib::particle::stationaryTetReverseTransform
(
	vector& centre,
	scalar& detA,
	barycentricTensor& T
...
void tnbLib::particle::movingTetReverseTransform
(
...
void tnbLib::particle::reflect()
{
	Swap(coordinates_.c(), coordinates_.d());
}


void tnbLib::particle::rotate(const bool reverse)
{
	if (!reverse)
	{
...
void tnbLib::particle::changeTet(const label tetTriI)
{
	if (debug)
	{
...
void tnbLib::particle::changeFace(const label tetTriI)
{
	if (debug)
	{
		Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
void tnbLib::particle::changeCell()
{
	if (debug)
	{
		Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
void tnbLib::particle::changeToMasterPatch()
{
	if (debug)
	{
		Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
tnbLib::particle::particle(const particle& p)
	:
	mesh_(p.mesh_),
...
tnbLib::particle::particle(const particle& p, const polyMesh& mesh)
	:
	mesh_(mesh),
	coordinates_(p.coordinates_),
	celli_(p.celli_),
...
tnbLib::scalar tnbLib::particle::track
(
...
tnbLib::scalar tnbLib::particle::trackToCell
(
	const vector& displacement,
	const scalar fraction
)
...
tnbLib::scalar tnbLib::particle::trackToFace
(
	const vector& displacement,
	const scalar fraction
)
...
tnbLib::scalar tnbLib::particle::trackToStationaryTri
(
	const vector& displacement,
	const scalar fraction,
	label& tetTriI
...
tnbLib::scalar tnbLib::particle::trackToMovingTri
(
	const vector& displacement,
	const scalar fraction,
	label& tetTriI
...
tnbLib::scalar tnbLib::particle::trackToTri
(
	const vector& displacement,
	const scalar fraction,
	label& tetTriI
...
tnbLib::vector tnbLib::particle::deviationFromMeshCentre() const
{
...
void tnbLib::particle::transformProperties(const tensor&)
{}


void tnbLib::particle::transformProperties(const vector&)
{}


void tnbLib::particle::prepareForParallelTransfer()
{
...
void tnbLib::particle::correctAfterParallelTransfer
(
	const label patchi,
	trackingData& td
)
...
void tnbLib::particle::prepareForInteractionListReferral
(
...
void tnbLib::particle::correctAfterInteractionListReferral(const label celli)
{
...
tnbLib::label tnbLib::particle::procTetPt
(
